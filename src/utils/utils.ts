...
export function addFieldInfoToTx(field_event: Event, tx_event: Event): string[] {
  let aux = <string[]>field_event.stakeholders;
  for(let i=0; i<aux.length; i++) {
    tx_event.stakeholders.push(aux[i]);
  }
  return <string[]>tx_event.stakeholders
}